...
#[cfg(test)]
mod tests {
    use crate::reed_solomon::{DecodeError, EncodeError, FieldOperation, RsDecoder, RsEncoder};
    ...
    impl Codec {
        fn new(num_roots: usize) -> Codec {
            Codec {
                enc: RsEncoder::new(CCSDS, 1, 1, num_roots),
                dec: RsDecoder::new(CCSDS, 1, 1, num_roots),
            }
        }
        ...
        // mirror the read-only fields a few tests check
    ...
    fn message_length_is_block_minus_distance() {
        let dec = RsDecoder::new(CCSDS, 1, 1, 32);
    ...
    fn stress_shuffle(a: &mut [usize], rng: &mut StressRng) {
        let len = a.len();
        for i in 0..len - 2 {
            let j = rng.below(len - i) + i;
            a.swap(i, j);
        }
    }
    ...
    fn ccsds_constructor_matches_explicit_params() {
        ...
        let mut a = RsEncoder::new_ccsds();
        let mut b = RsEncoder::new(0x187, 112, 11, 32);
